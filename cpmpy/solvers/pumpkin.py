...
class CPM_pumpkin(SolverInterface):
    ...
    def solve(self, time_limit=None, proof=None, assumptions=None, **kwargs):
        ...
        if self.has_objective():
            assert assumptions is None, (
                "Optimization under assumptions is not supported"
            )
        ...
        if self.has_objective():  # check result after optimisation
            ...
            if isinstance(result, SatisfactionResult.Satisfiable):
                ...
                raise ValueError(
                    f"Unknown Pumpkin-result: {result} of type {type(result)}, please report on github..."
                )
            ...
            for cpm_var in self.user_vars:
                ...
                if isinstance(sol_var, PumpkinInt):
                    ...
                    raise NotSupportedError(
                        "Only boolean and integer variables are supported."
                    )

            # translate solution values
            if self.has_objective():
                self.objective_value_ = solution.int_value(
                    self.solver_var(self._objective)
                )
    ...
    def solver_var(self, cpm_var):
        ...
        if is_num(cpm_var):
            ...
            if not cpm_var in self._constantvars:
                self._constantvars[cpm_var] = self.pum_solver.new_integer_variable(
                    cpm_var, cpm_var, name=str(cpm_var)
                )
        ...
        if cpm_var not in self._varmap:
            if isinstance(cpm_var, _BoolVarImpl):
                ...
                revar = self.pum_solver.new_integer_variable(
                    cpm_var.lb, cpm_var.ub, name=str(cpm_var)
                )
            else:
    ...
    def transform(self, cpm_expr):
        ...
        cpm_cons = reify_rewrite(
            cpm_cons, supported=supported_halfreif
        )  # reified element not supported yet (TODO?)
        cpm_cons = only_numexpr_equality(
            cpm_cons, supported=frozenset(["sum", "wsum", "sub"])
        )  # supports >, <, !=
        ...
        if isinstance(cpm_expr, _BoolVarImpl):
            ...
            if isinstance(lhs, Operator):  # can be sum with single arg
                if lhs.name == "sum" and len(lhs.args) == 1:
                    ...
                    raise ValueError(
                        "Lhs of predicate should be a sum with 1 argument"
                    )  # TODO: also wsum with 1 arg/mul with const?
            ...
            if cpm_expr.name == "<=":
                comp = Comparator.LessThanOrEqual
            if cpm_expr.name == ">=":
                comp = Comparator.GreaterThanOrEqual
            if cpm_expr.name == "!=":
                comp = Comparator.NotEqual
            if cpm_expr.name == "<":
                comp, rhs = Comparator.LessThanOrEqual, rhs - 1
            if cpm_expr.name == ">":
                comp, rhs = Comparator.GreaterThanOrEqual, rhs + 1

        else:
            raise ValueError(
                f"Cannot convert CPMpy expression {cpm_expr} to a predicate"
            )
    ...
    def _get_constraint(self, cpm_expr):
        ...
        if isinstance(cpm_expr, _BoolVarImpl):
            ...
            if cpm_expr.name == "or":
                ...
                return [
                    constraints.LessThanOrEquals(
                        self._sum_args(hacked, negate=True), -1
                    )
                ]
            ...
            assert isinstance(lhs, Expression), (
                f"Expected a CPMpy expression on lhs but got {lhs} of type {type(lhs)}"
            )
            ...
            if cpm_expr.name == "==":
                ...
                if lhs.name == "div":
                    ...
                    arr, idx = lhs.args
                    return [
                        constraints.Element(self._ivars(idx), self._ivars(arr), pum_rhs)
                    ]
                ...
                return [
                    constraints.LessThanOrEquals(self._sum_args(lhs, negate=True), -rhs)
                ]

            elif cpm_expr.name == ">":
                return [
                    constraints.LessThanOrEquals(
                        self._sum_args(lhs, negate=True), -rhs - 1
                    )
                ]
            elif cpm_expr.name == "!=":
            ...
            if cpm_expr.name == "alldifferent":
                ...
                assert all(is_num(d) for d in dur), (
                    "Pumpkin only accepts Cumulative with fixed durations"
                )
                assert all(is_num(d) for d in demand), (
                    "Pumpkin only accepts Cumulative with fixed demand"
                )
                assert is_num(cap), (
                    "Pumpkin only accepts Cumulative with fixed capacity"
                )

                return [
                    constraints.Cumulative(self.solver_vars(start), dur, demand, cap)
                ] + [
                    self._get_constraint(c)[0]
                    for c in self.transform(
                        [s + d == e for s, d, e in zip(start, dur, end)]
                    )
                ]
            else:
    ...
    def get_core(self):
        ...
        assert self._pum_core is not None, (
            "Can only get core if the last solve-call was unsatisfiable under assumptions"
        )
